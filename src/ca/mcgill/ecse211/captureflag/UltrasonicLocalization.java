...
/**
 * The UltrasonicLocalization class is used to orient the robot to 0 degrees. It is used only in the
 * beginning of the competition, when the robot is still in the corner.
 */
...
public class UltrasonicLocalization {
  ...
  /**
   * The falling and rising edges are detected in this method. The rising edge is when the sensor
   * goes from facing the wall to away from the wall and the falling edge is from away from the wall
   * to towards the wall. If no falling edge is detected and the distance drops below the constant
   * then a falling edge is said to have been detected. Then if a falling edge is detected and the
   * distance rises above our horizontal constant, a rising edge is said to be detected. The angle
   * return by the odometer is then saved. Then, the method uses both angles to determine by how
   * much the robot should turn to make it face north (0 degrees)
   */
   ...
   * 
   * @param distance the distance recorded by the US sensor
   */
  ...
  /**
   * Calculates a correction angle based on a rising edge formula. This method of localization is
   * preferred if the robot is facing a wall.
   */
  private void risingEdge() {
    rightMotor.rotate(-CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK, 360),
        true);
    leftMotor.rotate(CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK, 360),
        true);
    ...
    while (leftMotor.isMoving() && rightMotor.isMoving()) {
      ...
      if ((this.distanceUS >= HORIZONTAL_CONST - HORIZONTAL_MARGIN) && detectRaisingEdge == false
          && risingCounter == 0) {
        detectRaisingEdge = true;
        risingTheta = odometer.getTheta();
        risingCounter++;
        Sound.beep();
      } else if (this.distanceUS < HORIZONTAL_CONST + HORIZONTAL_MARGIN && detectRaisingEdge == true
          && risingCounter == 1) {
    ...
    rightMotor.rotate(CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK,
        deltaTheta * 180.0 / Math.PI), true);
    leftMotor.rotate(-CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK,
        deltaTheta * 180.0 / Math.PI), false);
  }


  /**
   * Calculates a correction angle based on a falling edge formula. This method of localization is
   * preferred if the robot is facing away from a wall.
   */
  private void fallingEdge() {
     ...
     * threshold, and if it is less than threshold, a falling edge is detected. Save the angle and
     * set the flag to true, so when the distance gets bigger than the threshold, the program enters
     * the second if statement and save the anlge of the rising edge.
     */
    ...
    leftMotor.rotate(-CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK,
        deltaTheta * 180 / Math.PI), true);
    rightMotor.rotate(CaptureFlag.convertAngle(CaptureFlag.WHEEL_RADIUS, CaptureFlag.TRACK,
        deltaTheta * 180 / Math.PI), false);
    odometer.setTheta(0);
