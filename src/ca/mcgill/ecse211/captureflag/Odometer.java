...
public class Odometer extends Thread {
  ...
  public void run() {
    ...
    while (true) {
      while (leftMotor.isMoving() || rightMotor.isMoving()) {
        ...
        double distR =
            Math.PI * CaptureFlag.WHEEL_RADIUS * (currTachoR - rightMotorTachoCount) / 180;
        double distL =
            Math.PI * CaptureFlag.WHEEL_RADIUS * (currTachoL - leftMotorTachoCount) / 180;
  ...
  /**
   * Method assigns position array elements with x, y, and orientation values currently being read
   * from odometer if the corresponding boolean array element is true
   * 
   * @param position Array that contains x and y position and angle oriented to
   * @param update Boolean array that allows values to be assigned to position array if element is
   *        true
   */
